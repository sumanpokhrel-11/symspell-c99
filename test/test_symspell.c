...
void test_lookup(symspell_dict_t* dict) {
    /* symspell_lookup() no longer takes a verbosity mode: it always returns
     * suggestions at the closest edit distance, capped by max_suggestions. */
    printf("\n=== Testing Lookup ===\n\n");
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        int count = symspell_lookup(dict, test_words[i], 2, suggestions, 10);
        if (count > 0) {
...
void test_edge_cases(symspell_dict_t* dict) {
    ...
    count = symspell_lookup(dict, "hello", 2, suggestions, 10);
    printf("   'hello' → ");
...
void test_specific_cases(symspell_dict_t* dict) {
    ...
    count = symspell_lookup(dict, "helo", 2, suggestions, 10);
    ...
    count = symspell_lookup(dict, "teh", 2, suggestions, 10);
    if (count > 0) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 2) {
        ...
        while (1) {
            ...
            int count = symspell_lookup(dict, input, 2, suggestions, 5);
        ...
        for (int i = 2; i < argc; i++) {
            ...
            int count = symspell_lookup(dict, argv[i], 2, suggestions, 1);
